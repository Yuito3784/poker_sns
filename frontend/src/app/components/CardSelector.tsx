...
// 上段 / 下段に分けて、スマホでも「2」だけ段落ちしないようにする
      ...
      {open && (
        <>
          ...
          >
            ...
            {/* Card grid */}
            <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 6, maxHeight: "60vh", overflowY: "auto" }}>
              {suits.map((suit) => (
                <div key={suit.value} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {/* Rank buttons（2段レイアウト） */}
                  <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    {rankRows.map((row, rowIdx) => (
                      <div key={rowIdx} style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                        {row.map((rank) => {
