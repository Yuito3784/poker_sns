      ...
      >
        ...
        {typeof navigator !== "undefined" && "share" in navigator && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              const url = `${window.location.origin}/post/${post.id}?utm_source=pokersns&utm_medium=share&utm_campaign=post_share`;
              const text = post.isPokerHand
                ? "ポーカーハンド"
                : (post.content?.slice(0, 60) ?? "Poker SNS");
              navigator.share({ title: "Poker SNS", text, url }).catch(() => {});
        ...
        >
          {copiedPostId === post.id ? (
            <svg className="h-[17px] w-[17px]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} style={{ color: "#c9a84c" }}>
        ...
        <a
          ...
          title="Xでシェア"
        >
          <svg className="h-[17px] w-[17px] transition-transform group-hover:scale-110" viewBox="0 0 24 24" fill="currentColor">
        ...
        <a
          href={(() => {
            const url = typeof window !== "undefined"
              ? `${window.location.origin}/post/${post.id}?utm_source=pokersns&utm_medium=share&utm_campaign=post_share`
              : "";
            return `https://discord.com/channels/@me?content=${encodeURIComponent(url)}`;
          })()}
          target="_blank"
          rel="noopener noreferrer"
