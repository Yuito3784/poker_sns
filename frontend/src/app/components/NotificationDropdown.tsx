...
export default function NotificationDropdown({ notifications, token, onClose, onRefresh }: Props) {
  const handleClick = async (notif: Notification) => {
    ...
    if ((notif.type === "LIKE" || notif.type === "REPLY" || notif.type === "MENTION" || notif.type === "REPOST") && notif.postId) {
      ...
      window.location.href = `/profile/${notif.fromUser.username}`;
    }
  };
  ...
  return (
    <div className="divide-y divide-neutral-200">
      {notifications.length === 0 ? (
        <p className="p-4 text-center text-sm text-neutral-600">通知はありません</p>
      ) : (
        notifications.map((notif) => (
          <button
            key={notif.id}
            className={`w-full p-3 text-left text-sm hover:bg-neutral-50 ${!notif.isRead ? "bg-blue-50" : ""}`}
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1">
                ...
                {notif.post && (
                  <p className="mt-1 text-xs text-neutral-600 line-clamp-2">{notif.post.content}</p>
                )}
                <p className="mt-1 text-xs text-neutral-500">{formatRelativeTime(notif.createdAt)}</p>
              </div>
            </div>
          </button>
        ))
