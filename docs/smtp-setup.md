...
**Gmail で届かないときの確認:**
- **迷惑メールフォルダ**を確認する。
- **アプリパスワード**は 16 文字を**スペースなし**でコピーしているか（Google の表示は 4 文字ずつ区切られているが、実際の値は続きの 16 文字）。
- **2 段階認証**が有効になっているか（オフだとアプリパスワードは作成できない）。
- **Railway のログ**で「Verification email send failed」が出ていないか確認する。出ている場合はその直後のメッセージ（認証エラー・接続エラーなど）で原因を判断できる。
- Google 側で「安全性の低いアプリのアクセス」を有効にする必要は**ない**（アプリパスワードを使う場合は不要）。
- **Railway などで「送信中」が長く続き、ログに `ENETUNREACH 2404:6800:...` と出る場合:** ホスト側が IPv6 で Gmail に接続しようとして失敗しています。バックエンドでは起動時に IPv4 優先（`dns.setDefaultResultOrder('ipv4first')`）と SMTP の接続タイムアウト（15 秒）を設定しているため、再デプロイ後は早めに失敗するか IPv4 で送信できるようになります。
